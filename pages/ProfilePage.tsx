...
import { updateUserProfile, resetUserProgress, setupPushNotifications } from '../services/firestoreService';
...
const ProfilePage = () => {
    ...
    const handleNameSave = async () => {
        if (editedName.trim()) {
            await updateUserProfile(user.id, { name: editedName.trim() });
    ...
    const handlePushToggle = async () => {
        ...
        await setupPushNotifications(user.id, newState);
        if (newState && Notification.permission === 'denied') {
            alert("Les notificacions estan bloquejades al navegador. Canvia la configuració per permetre-les.");
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
                    ...
                    <div className="flex-grow">
                        {isEditingName ? (
                            ...
                            <h3 className="text-xl font-bold text-slate-800 flex items-center">
                                {user.name}
                                <button onClick={() => setIsEditingName(true)} className="ml-3 text-slate-500 hover:text-sky-600">
                ...
                {isEditingName && (
                    <div className="flex justify-end space-x-2 mt-4">
                        <button onClick={() => setIsEditingName(false)} className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200">Cancel·lar</button>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
                 ...
                 <ul className="divide-y divide-slate-200">
                    <li className="flex items-center justify-between py-3">
                        <span className="text-slate-600">Notificacions Push</span>
                        <button onClick={handlePushToggle} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${pushNotifications ? 'bg-sky-600' : 'bg-slate-300'}`}>
                 ...
                 <div className="space-y-4">
                    <div className="flex items-center justify-between">
                     ...
                     <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                        ...
                        <button onClick={handleLogout} className="px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-200 rounded-lg hover:bg-slate-300 flex-shrink-0">
                            Tancar sessió
                        </button>
                    </div>
                 </div>
